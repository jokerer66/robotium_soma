...
public class Testlooclick extends ActivityInstrumentationTestCase2 {
...
//		08-29 10:37:54.104: E/tempview(11445): com.instanza.baba.activity.CocoVoice\
	...
	public void test_loopclick(){
		...
		for(int i=100 ;i<allviewlist.size();i++){
			...
			// view若为listview时，且可点击时，对其子item进行点击遍历
				...
				if((location[0] == 1104 && location[1] == 98) ){
					oldactivity = solo.getCurrentActivity();
					...
					solo.clickOnView(tempview);solo.sleep(5000);
					newtextlist = solo.getCurrentViews(TextView.class);
					for(int zz= 2;zz<newtextlist.size();zz++){
						Log.e("tempview", newtextlist.get(zz).getText().toString());
						if(newtextlist.get(zz).getText().toString().length()>=4 && newtextlist.get(zz).isShown()){
							
							newpagename = newtextlist.get(zz).getText().toString();
							Log.e("tempview", "newpagename = " + newpagename);
							break;
						}
					}
